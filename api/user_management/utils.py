...
def get_user_login_details(user_obj):
    """
    :param user_obj:
    :return:
    """
    ...
    user_data['access_token'] = access_token(
        user_id=user_obj.id, user_email=user_obj.email)
    ...
    if user_obj:
        if user_obj.deleted_at:
            ...
            if decrypt_password == password:
                ...
                user_data = get_user_login_details(user_obj=user_obj)
                message = success_msg
                status_code = 200
            else:
...
def brand_register(db, brand_name, competitor_name, hashtag):
    """
    :param db:
    :param brand_name:
    ...
    :return:
    """
    brand_obj = brands(brand_name=brand_name,
                       competitor_name=competitor_name, hashtag=hashtag)
...
def brands_listing(db, account_type):
    """
    :param db:
    :param account_type:
    :return:
    """
    brand_listing = []
    brands_listing_obj = db.query(brands).filter(
        brands.account_type == account_type).all()
    print("brands_listing_obj", brands_listing_obj)
