...
def add_brand(db, brand_competitor_hashtag_keywords):
    """
    ...
    try:
        ...
        message, status_code = brand_register(db=db, brand_name=brand_name, competitor_name=competitor_name, hashtag=hashtag)
...
def fetch_brands_listing(db, account_type):
    """
    ...
    """
    try:
        message, status_code, data = brands_listing(db, account_type)
    except Exception as exc:
        logging.exception(str(exc))
        traceback.print_exc()
        message = exception_msg
